...
/**
 * WMACS MCP Server Integration
 * 
 * Proper MCP server for WMACS operations with LDC Construction Tools
...
class WMACsMCPServer {
  ...
  loadConfig() {
    try {
      ...
      console.error('❌ Failed to load WMACS config:', error.message);
      process.exit(1);
    }
  }

  async restartLDCApplication(environment) {
    console.log(`🛡️ WMACS MCP: Restarting LDC application on ${environment}`);
    ...
    try {
      ...
      console.log(`✅ WMACS MCP: Application restarted on container ${env.container}`);
      ...
      console.error(`❌ WMACS MCP: Failed to restart application:`, error.message);
      throw error;
    }
  }

  async testLDCEnvironment(environment) {
    console.log(`🛡️ WMACS MCP: Testing LDC environment ${environment}`);
    ...
    try {
      ...
      console.log(`✅ WMACS MCP: Environment test completed - ${result.message}`);
      return result;
    } catch (error) {
      console.error(`❌ WMACS MCP: Environment test failed:`, error.message);
      return {
        success: false,
        environment,
        error: error.message
  ...
  async deployToProduction() {
    console.log('🛡️ WMACS MCP: Starting production deployment');
    
    try {
      // Step 1: Merge staging to main
      console.log('   • Merging staging to main...');
      execSync('git checkout main && git merge staging --no-ff -m "feat: WMACS MCP deployment to production"', { stdio: 'inherit' });
      ...
      console.log('✅ WMACS MCP: Production deployment completed');
      return {
        success: true,
      ...
      console.error('❌ WMACS MCP: Production deployment failed:', error.message);
      throw error;
    }
  }

  async validateDeployment(environment) {
    console.log(`🛡️ WMACS MCP: Validating deployment on ${environment}`);
    ...
    try {
      ...
      console.log(`✅ WMACS MCP: Validation completed - ${allSuccessful ? 'All tests passed' : 'Some tests failed'}`);
      ...
      console.error(`❌ WMACS MCP: Validation failed:`, error.message);
      throw error;
    }
  }
  ...
  async executeOperation(operation, ...args) {
    ...
    console.log(`🛡️ WMACS MCP: Executing operation '${operation}' with args:`, args);
    
    try {
      const result = await this.operations[operation](...args);
      console.log(`✅ WMACS MCP: Operation '${operation}' completed successfully`);
      return result;
    } catch (error) {
      console.error(`❌ WMACS MCP: Operation '${operation}' failed:`, error.message);
      throw error;
    }
  }
...
if (require.main === module) {
  ...
  if (!operation) {
    console.log('🛡️ WMACS MCP Server - Available Operations:');
    server.listOperations().forEach(op => console.log(`   • ${op}`));
    console.log('\nUsage: node wmacs-mcp-server.js <operation> [args...]');
    process.exit(0);
  }

  server.executeOperation(operation, ...args)
    .then(result => {
      console.log('🎉 WMACS MCP: Operation result:', JSON.stringify(result, null, 2));
      process.exit(0);
    })
    .catch(error => {
