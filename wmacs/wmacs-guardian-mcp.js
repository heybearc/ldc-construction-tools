...
class WMACSGuardianMCP {
  ...
  async diagnoseProduction() {
    console.log('🛡️ WMACS Guardian MCP: Diagnosing production issues');
  ...
  async forceRestart() {
    console.log('🛡️ WMACS Guardian MCP: Force restarting production service');
  ...
  async healthCheck() {
    console.log('🛡️ WMACS Guardian MCP: Comprehensive health check');
...
if (require.main === module) {
  const guardian = new WMACSGuardianMCP();
  ...
  if (!operation || !operations[operation]) {
    console.log('🛡️ WMACS Guardian MCP - Available Operations:');
    Object.keys(operations).forEach(op => console.log(`   • ${op}`));
    console.log('\nUsage: node wmacs-guardian-mcp.js <operation> [args...]');
    process.exit(0);
  }

  operations[operation](...args)
    .then(result => {
      console.log('🎉 WMACS Guardian MCP: Result:', JSON.stringify(result, null, 2));
      process.exit(0);
    })
    .catch(error => {
      console.error('❌ WMACS Guardian MCP: Error:', error.message);
      process.exit(1);
    });
}

module.exports = WMACSGuardianMCP;
