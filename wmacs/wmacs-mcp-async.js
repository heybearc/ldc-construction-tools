...
/**
 * WMACS MCP Async Server
 * 
 * Non-blocking MCP server for WMACS operations
...
class WMACsMCPAsync {
  ...
  async quickTest(environment) {
    console.log(`🛡️ WMACS MCP Async: Quick test ${environment}`);
  ...
  async restartService(environment) {
    console.log(`🛡️ WMACS MCP Async: Restart service ${environment}`);
  ...
  async getStatus() {
    console.log('🛡️ WMACS MCP Async: Getting system status');
  ...
  async deploymentSummary() {
    console.log('🛡️ WMACS MCP Async: Deployment summary');
...
if (require.main === module) {
  ...
  if (!operation || !operations[operation]) {
    console.log('🛡️ WMACS MCP Async Server - Available Operations:');
    Object.keys(operations).forEach(op => console.log(`   • ${op}`));
    console.log('\nUsage: node wmacs-mcp-async.js <operation> [args...]');
    process.exit(0);
  }

  operations[operation](...args)
    .then(result => {
      console.log('🎉 WMACS MCP Async: Result:', JSON.stringify(result, null, 2));
      process.exit(0);
    })
    .catch(error => {
      console.error('❌ WMACS MCP Async: Error:', error.message);
      process.exit(1);
    });
}
