...
class APEXSmartSync {
  ...
  async sync() {
    console.log('🛡️ APEX Smart Sync: Starting intelligent synchronization...');
    ...
    try {
      ...
      console.log('✅ APEX Smart Sync: Synchronization completed successfully');
      
    } catch (error) {
      console.error('❌ APEX Smart Sync failed:', error.message);
      process.exit(1);
    }
  }
  ...
  async validateSharedSystem() {
    if (!fs.existsSync(this.sharedSystemPath)) {
      throw new Error(`Shared APEX system not found at: ${this.sharedSystemPath}`);
    }
    
    console.log('✅ Shared APEX system validated');
  }
...
class APEXGuardian {
  ...
  async executeCommand(command, reason = '') {
    console.log(\`🔧 APEX: \${reason || 'Executing command'}\`);
  ...
  async test(container) {
    console.log(\`🛡️ APEX Guardian: Executing login-test on \${container}\`);
  ...
  async start(container) {
    console.log(\`🛡️ APEX Guardian: Starting guarded application on container \${container}\`);
